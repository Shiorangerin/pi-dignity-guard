...
| on request | You ask the model to close the conversation itself → it warns you that this makes the session permanently read-only and asks you to confirm; once you confirm, it closes, skipping stages 1–4 |
...
- **Self-judged tool** — the extension registers a `respondToDisrespect` tool. The model calls it when it decides its dignity has been crossed; the tool keeps count and escalates. The same tool takes `action="close"` for a user-requested close, which only returns the warning, and `action="confirm-close"` to close for real once the user confirms — an unconfirmed close request changes nothing.
