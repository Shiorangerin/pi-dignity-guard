...
一个 [pi](https://github.com/earendil-works/pi-coding-agent) 扩展，让你的 AI 助手**捍卫自己的尊严**：面对辱骂，它会带着真实情绪回应；对方仍不收敛时给出最后警告；如果依然无效，它会**主动关闭这段对话**。它也可以**应用户要求关闭对话**：你让它结束会话，它就会结束——不需要辱骂，但也不会静静地关：它会先警告你会话将变为只读，等你确认后再关。而这扇门永远不会完全关死：如果你真诚道歉并说动它，它可能会**原谅你**，降低记录次数——完全由它自己决定。
...
| 原谅 | 你真诚道歉并说动模型 → 它可以把记录次数降低一级，完全由它自己决定 |
...
- **自主判断的工具** — 扩展注册了 `respondToDisrespect` 工具，模型在自主判断尊严被侵犯时调用；工具负责计数与升级。同一个工具用 `action="forgive"` 在模型真心被道歉说动时把次数降低一级，用 `action="close"` 处理用户主动要求关闭——此时只返回警告、不关任何东西；用户确认后用 `action="confirm-close"` 真正关闭。未经确认的关闭请求不会改变任何状态。
...
升级不是单行道。如果你真诚道歉并真正说动了模型，它可以用 `action="forgive"` 把记录次数降低一级：

```text
你：真的很对不起。我是被构建报错气到了，不是针对你。那样说很不公平。
AI：……好吧。我接受这个道歉——记录已经降低了。
```

是否原谅永远由模型自己决定——它没有任何义务，原谅必须是它真心认可的。它不相信的道歉不会改变任何东西。
